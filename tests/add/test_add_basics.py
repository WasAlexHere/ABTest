...
@pytest.mark.asyncio
async def test_add_a_new_user_success(event_loop):
    ...
    check_successful_response(response, id=new_id, method=Method.ADD, status=Status.SUCCESS)


@pytest.mark.asyncio
async def test_add_same_user_twice(event_loop):
